...
async function buildTeamFormationSvg(team, dateStr = '', timeRange = '', options = {}) {
  ...
  const renderSinglePlayer = (player, posCode, isAlternate, isMom, cx, cy) => {
    ...
    if (pHasWRating) {
      ...
      ratingBadgeSvg = `
        <g transform="translate(${pillX}, ${pillY})">
          ...
          ${isMom ? `<g transform="translate(-16, -6) scale(1.5)"><polygon points="53,8 55.5,14 62,14 57,18 59,24 53,20 47,24 49,18 44,14 50.5,14" fill="#FDE047"/></g>` : ''}
        </g>
      `;
    }
    ...
    const goalBadgeSvg = goals > 0 ? `
      <g transform="translate(42, 32)">
        ...
        ${goalIconDataUri ? `
          <image href="${goalIconDataUri}" xlink:href="${goalIconDataUri}" x="-21" y="-21" width="42" height="42" preserveAspectRatio="xMidYMid meet"/>
        ` : `
    ...
    const crownSvg = isMom
      ? `<g transform="translate(-34, -102) scale(0.135)">${CROWN_PATHS}</g>`
      : '';
    ...
    if (momPlayer) {
      ...
      mvpBarSvg = `
        ...
        <g transform="translate(40, 1390)">
          ...
          <text x="303" y="89" font-size="30" font-family="Sarabun, sans-serif" font-weight="bold" fill="#FFFFFF">• ${escapeXml(momName)}</text>
          ${momStatsSvg}

          <!-- Rating Box -->
          <rect x="810" y="30" width="150" height="90" rx="14" fill="#231602" stroke="#F59E0B" stroke-width="2"/>
  ...
  <g transform="translate(40, 24)">
    ...
    <!-- Date & Time -->
    <text x="${svgWidth - 240}" y="42" font-size="28" font-family="Sarabun, sans-serif" font-weight="bold" fill="#CBD5E1">${escapeXml(WeekDate)}</text>
    <text x="${svgWidth - 250}" y="80" font-size="28" font-family="Sarabun, sans-serif" font-weight="bold" fill="#CBD5E1">${escapeXml(timeRange)}</text>
  ...
  <!-- Footer Watermark -->
  <text x="${svgWidth / 2}" y="${svgHeight - 20}" font-size="25" font-family="Sarabun, sans-serif" fill="#64748B" text-anchor="middle">Generated by AgentVII • ${new Date().getFullYear()}</text>
  `;
